...
# 生成梯度分布数据（精确匹配图示形态）
def generate_gradients(peak_pos, size):
    base = np.concatenate([
        np.random.normal(peak_pos, 0.1, size//2),
        np.random.exponential(scale=0.5, size=size//2)
    ])
    return np.clip(base, 0, 3)

grad_early = generate_gradients(0.2, 5000)  # 早期阶段
grad_mid = generate_gradients(1.0, 5000)    # 中期阶段
grad_late = generate_gradients(2.0, 5000)    # 后期阶段
...
for data, color, label in zip([grad_early, grad_mid, grad_late], colors, labels):
    ...
    y = kde(x) * 16 / kde(x).max()  # 确保峰值达到16
    ax1.plot(x, y, color=color, lw=2.5, label=label)
...
ax2.set(xlim=(2,9), ylim=(0,0.36),
       xticks=epochs,
       yticks=[0,0.1,0.2,0.3,0.35],
       xlabel='训练轮次',
       ylabel='KL散度')
...
# ==================== 保存输出 ====================
plt.tight_layout()
plt.savefig('gradient_analysis_precise.png',
           bbox_inches='tight',
           dpi=300,
           facecolor='white')
print("精确复现图表已保存：gradient_analysis_precise.png")